...
docker run -it -d --restart=always   --network=host --privileged  --hostname=ovn-run_nb_ovsdb --env-file=/root/ovn/env.ovn-run_nb_ovsdb.list.${NODEID} --name ovn-run_nb_ovsdb  -v /root/ovn/log:/var/log/ovn/ -v /root/ovn/lib:/var/lib/ovn -v /root/ovn/run:/var/run/ovn ovn:v9

docker run -it -d --restart=always  --network=host --privileged  --hostname=ovn-run_sb_ovsdb --env-file=/root/ovn/env.ovn-run_sb_ovsdb.list.${NODEID} --name ovn-run_sb_ovsdb -v /root/ovn/log:/var/log/ovn/ -v /root/ovn/lib:/var/lib/ovn -v /root/ovn/run:/var/run/ovn ovn:v9

docker run -it -d --restart=always  --network=host --privileged  --hostname=ovn-northd  --env-file=/root/ovn/env.ovn-northd.list.${NODEID} --name ovn-northd -v /root/ovn/log:/var/log/ovn/ -v /root/ovn/lib:/var/lib/ovn -v /root/ovn/run:/var/run/ovn ovn:v9
