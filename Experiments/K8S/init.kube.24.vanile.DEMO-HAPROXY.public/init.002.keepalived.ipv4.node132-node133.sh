...
export VIPIP_KEEPALIVE=192.168.200.187
...
vrrp_instance VI_1 {
    ...
    unicast_peer {
       ...
       $(if [ $NODEID -ne 133 ]; then echo "${KEEPALIVED_NET}.133";fi;)
    }
    track_script {
        check_apiserver
    }
...
apt install -y haproxy || throw ${LINENO}
...
backend kubernetes-backend
  ...
  balance roundrobin
    server master-1 192.168.200.180:6443 check fall 3 rise 2
    server master-2 192.168.200.181:6443 check fall 3 rise 2
    server master-3 192.168.200.182:6443 check fall 3 rise 2
EOF

systemctl restart haproxy.service || throw ${LINENO}
systemctl status haproxy.service || throw ${LINENO}
