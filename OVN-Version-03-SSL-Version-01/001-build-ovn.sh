...
apt install sshpass docker.io  iproute2 net-tools iputils-ping iperf3 wget -y
...
case \$OVNCMD in
        ...
        ;;
        "ovs-controller") ovn-controller unix:/var/run/openvswitch/db.sock --no-chdir --monitor  --overwrite-pidfile \
                         --log-file=/var/log/openvswitch/ovn-controller.log \
                         --pidfile=/var/run/openvswitch/ovn-controller.pid
        ;;

        "ovs-controller-ssl") ovn-controller unix:/var/run/openvswitch/db.sock --no-chdir --monitor  --overwrite-pidfile \
                         --log-file=/var/log/openvswitch/ovn-controller.log \
                         --private-key=/etc/ovn/ovn-privkey.pem \
                         --certificate=/etc/ovn/ovn-cert.pem \
                         --ca-cert=/etc/ovn/cacert.pem \
                         --pidfile=/var/run/openvswitch/ovn-controller.pid
        ;;
        ...
        ;;
        "ovn-run_nb_ovsdb-host-ssl") /usr/share/ovn/scripts/ovn-ctl run_nb_ovsdb \
                        --db-nb-addr=\${OVNHOSTIP} \
                        --db-sb-addr=\${OVNHOSTIP} \
                        --db-nb-cluster-local-addr=\${OVNHOSTIP} \
                        --db-sb-cluster-local-addr=\${OVNHOSTIP} \
                        --ovn-northd-nb-db=ssl:\${OVNHOSTIP}:6641 \
                        --ovn-northd-sb-db=ssl:\${OVNHOSTIP}:6642 \
                        --ovn-nb-db-ssl-key="/etc/ovn/ovn-privkey.pem" \
                        --ovn-nb-db-ssl-cert="/etc/ovn/ovn-cert.pem" \
                        --ovn-nb-db-ssl-ca-cert="/etc/ovn/cacert.pem"
        ;;
        "ovn-run_sb_ovsdb-host-ssl") /usr/share/ovn/scripts/ovn-ctl run_sb_ovsdb \
                        --db-nb-addr=\${OVNHOSTIP} \
                        --db-sb-addr=\${OVNHOSTIP} \
                        --db-nb-cluster-local-addr=\${OVNHOSTIP} \
                        --db-sb-cluster-local-addr=\${OVNHOSTIP} \
                        ...
                        --ovn-nb-db-ssl-ca-cert="/etc/ovn/cacert.pem"
        ;;
        "ovn-run_nb_ovsdb-cluster-ssl") /usr/share/ovn/scripts/ovn-ctl run_nb_ovsdb \
                        --db-nb-addr=\${OVNHOSTIP} \
                        --db-sb-addr=\${OVNHOSTIP} \
                        --db-nb-cluster-local-addr=\${OVNHOSTIP} \
                        --db-sb-cluster-local-addr=\${OVNHOSTIP} \
                        ...
                        --ovn-nb-db-ssl-ca-cert="/etc/ovn/cacert.pem"
        ;;
        "ovn-run_nb_ovsdb-cluster-join-ssl") /usr/share/ovn/scripts/ovn-ctl run_nb_ovsdb \
                        --db-nb-addr=\${OVNHOSTIP} \
                        --db-sb-addr=\${OVNHOSTIP} \
                        --db-nb-cluster-local-addr=\${OVNHOSTIP} \
                        --db-sb-cluster-local-addr=\${OVNHOSTIP} \
                        ...
                        --ovn-nb-db-ssl-ca-cert="/etc/ovn/cacert.pem"
        ;;
        "ovn-run_sb_ovsdb-cluster-ssl") /usr/share/ovn/scripts/ovn-ctl run_sb_ovsdb \
                        --db-nb-addr=\${OVNHOSTIP} \
                        --db-sb-addr=\${OVNHOSTIP} \
                        --db-nb-cluster-local-addr=\${OVNHOSTIP} \
                        --db-sb-cluster-local-addr=\${OVNHOSTIP} \
                        ...
                        --ovn-nb-db-ssl-ca-cert="/etc/ovn/cacert.pem"
        ;;
        "ovn-run_sb_ovsdb-cluster-join-ssl") /usr/share/ovn/scripts/ovn-ctl run_sb_ovsdb \
                        --db-nb-addr=\${OVNHOSTIP} \
                        --db-sb-addr=\${OVNHOSTIP} \
                        --db-nb-cluster-local-addr=\${OVNHOSTIP} \
                        --db-sb-cluster-local-addr=\${OVNHOSTIP} \
                        ...
                        --ovn-nb-db-ssl-ca-cert="/etc/ovn/cacert.pem"
        ;;
        "ovn-northd-host-ssl") ovn-northd  --monitor  --overwrite-pidfile \
                                --private-key=/etc/ovn/ovn-cert.pem --certificate=/etc/ovn/ovn-privkey.pem --ca-cert=/etc/ovn/cacert.pem \
                                --ovnnb-db=ssl:\${OVNHOSTIP}:6641 \
                                --ovnsb-db=ssl:\${OVNHOSTIP}:6642 \
                                --no-chdir --log-file=/var/log/ovn/ovn-northd.log --pidfile=/var/run/ovn/ovn-northd.pid
        ;;
        "ovn-northd-cluster-ssl") ovn-northd  --monitor  --overwrite-pidfile \
                                --private-key=/etc/ovn/ovn-cert.pem \
                                --certificate=/etc/ovn/ovn-privkey.pem \
                                --ca-cert=/etc/ovn/cacert.pem \
        ...
        *)  echo "$0 [ovsdb-init|ovsdb-server|ovs-vswitchd|ovs-controller|ovn-run_sb_ovsdb-host|ovn-run_nb_ovsdb-host|ovn-run_sb_ovsdb-cluster|ovn-run_nb_ovsdb-cluster|ovn-northd|ovn-ic-host|ovn-ic-cluster|ovn-run_ic_nb_ovsdb-host|ovn-run_ic_sb_ovsdb-host|ovn-run_ic_nb_ovsdb-cluster|ovn-run_ic_sb_ovsdb-cluster|ovn-run_nb_ovsdb-cluster-join|ovn-run_sb_ovsdb-cluster-join]"
            ...
            echo "OVNCLUSTER6641  on OVN SSL NB CENTRAL - ssl:x.x.x.x:6641,ssl:y.y.y.y:6642,ssl:z.z.z.z:6641"
            echo "OVNCLUSTER6642  on OVN SSL SB CENTRAL - ssl:x.x.x.x:6642,ssl:y.y.y.y:6641,ssl:z.z.z.z:6642"
esac
EOF
